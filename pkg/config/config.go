...
type core struct {
	// The public master URL of your DC/OS installation
	URL string `toml:"dcos_url"`

	// The token generated by authenticating to DC/OS with ACS
	ACSToken string `toml:"dcos_acs_token"`

	// Trust policy for HTTPS CAs.
	SSL ssl `toml:"ssl_verify"`

	// Request timeout in seconds (while waiting for a response once the connection is established)
	...
	// Whether to prompt the user to log in when token expired; otherwise automatically initiate login
...
func (cfg Config) StringAt(key []string) (string, error) {
	...
	case *toml.Tree:
		...
		tomlutil.VisitLeaves(value, key, func(leaf interface{}, nodeKeys []string) {
